...
1. 仕様を策定する
  - 仕様は `/docs` にドキュメント化する
  - ドキュメントは、Obsidianを通じて管理されているため、その形式に準じて統一する
2. 作業計画を立てる
  - 作業計画は `/docs/backlog` にドキュメント化する
    - ファイル名は `YYYY-MM-DD_{作業内容}.md` とする
  - 作業計画を立てたらGeminiと人間のレビューを受ける
2. **作業ブランチ作成**
  - ブランチ名は `feature/YYYY-MM-DD_作業内容` とする
3. **開発作業**
  - 一連の作業は工程ごとに区切って行う
    - 工程ごとに静的解析、型チェックを行う
    - 工程ごとにコミットする
  ...
  - コード内のコメントは必要な場合に書く
    - 見てわかるようなコメントは不要
  - `cd` コマンドを使うときは現在のディレクトリを確認する
  - ライブラリやAPIを利用するときは経験則で実装せずにGeminiや人間に壁打ちをして実装する
4. **Pull Request作成**
  - PRの概要に実装について記載し人間のレビューを受ける
5. **PRをマージする**
  - 人間から承認を得たらマージを行う
...
- **ファイル命名規則**: テーブル名.ts（例：`customer.ts`, `contract.ts`）

#### ENUM型の使用
- **ステータス管理**: status系のフィールドは必ずENUM型で定義
...
- **型安全性**: Drizzle ORMの型推論を活用

#### スキーマ統合ファイル
- **index.ts**: 依存関係順序を考慮した統合エクスポート
- **Biome警告**: import順序警告は依存関係を重視して無視
- **コメント**: 順序の理由を明記

#### データベース正規化
- **関心の分離**: 顧客、契約、店舗、招待、ユーザーを独立したテーブルに
