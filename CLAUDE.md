...
## 開発フェーズ
1. **Phase 1**: 共通バックエンド構築・基本認証 ✅ 完了
2. **Phase 2**: 運営側管理サイト（DB・認証・基本画面） ✅ 完了
3. **Phase 3**: 店舗経営者管理サイト基盤（Google OAuth・基本機能）
4. **Phase 4**: 店舗サイト自動生成（Next.js・SEO対応）
5. **Phase 5**: 決済・課金システム（PAY.JP連携）
6. **Phase 6**: 高度な機能（Google Maps・MEO・口コミ管理）

## ユーザーフロー設計
詳細なユーザーフローは `docs/USER_FLOWS.md` を参照してください。
...
### 現在の開発状況
- ✅ 運営管理サイト：ログイン、ダッシュボード、基本管理画面
- ❌ 店舗経営者管理サイト：未作成 (`packages/store-admin`)
- ❌ 店舗サイト：未作成 (`packages/store-site`)
